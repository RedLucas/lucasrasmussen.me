...
//
// Deliberately returns its own inferred shape rather than casting to
// WebGL2RenderingContext — tests need `.drawArrays.mock.calls` etc., which
// the real interface's plain method signatures don't expose. The cast to
// the full interface happens only at the `getContext` stub boundary below,
// where the component code under test actually receives it.
...
export type MockGLContext = ReturnType<typeof createMockGLContext>;

export interface CanvasGetContextStub {
  contexts: MockGLContext[];
  restore: () => void;
}
...
export function stubCanvasGetContext(): CanvasGetContextStub {
  const contexts: MockGLContext[] = [];
  ...
  function stubbedGetContext(
    ...
    if (contextId !== 'webgl2' && contextId !== 'webgl') {
      return (original as (...args: unknown[]) => unknown).apply(this, [contextId, ...rest]);
    }
  ...
  const spy = vi
    ...
    .mockImplementation(
      stubbedGetContext as unknown as typeof HTMLCanvasElement.prototype.getContext,
    );
